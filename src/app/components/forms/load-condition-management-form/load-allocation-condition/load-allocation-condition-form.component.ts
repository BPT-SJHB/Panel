...
import { LoadAllocationConditionInfo } from 'app/services/config-management/model/load-allocation-condition-info.model';
...
/* ----------------------------- INTERFACES ---------------------------------- */
type LoadAllocationConditionRow = LoadAllocationConditionInfo & {
  delete: string;
  edit: string;
};
...
/* -------------------------------- COMPONENT -------------------------------- */
@Component({
  selector: 'app-load-allocation-condition-form',
  templateUrl: './load-allocation-condition-form.component.html',
  styleUrl: './load-allocation-condition-form.component.scss',
  imports: [
    TableComponent,
...
export class LoadAllocationConditionFormComponent extends BaseLoading {
  /* -------------------------- INJECTED SERVICES -------------------------- */
  ...
  /* ---------------------------- STATE & SIGNALS --------------------------- */
  readonly loadsAllocations = signal<LoadAllocationConditionRow[]>([]);
  readonly dialogTitle = signal<string>('');
  readonly formMode = signal<FormMode>(FormMode.REGISTER);
  readonly selectedLoadAllocations = signal<LoadAllocationConditionInfo | null>(
    null
  );
  ...
  readonly form = this.fb.group({
    LoadAllocationConditionId: this.fb.control<number | null>(null),
    ...
    TurnStatusId: this.fb.control<number | null>(null, ValidationSchema.id),
    TurnStatusTitle: this.fb.nonNullable.control<string>(''),
  });

  /* ------------------------------- TABLE CONFIG ------------------------------ */
  readonly columns: TableColumn<LoadAllocationConditionRow>[] = [
    { header: 'شناسه', field: 'LoadAllocationConditionId' },
    ...
    { field: 'TurnStatusTitle', header: 'وضعیت نوبت' },
    {
      ...editCell.config,
      field: 'edit',
      onAction: (row: LoadAllocationConditionInfo) => this.onEdit(row),
    },
    {
      ...deleteCell.config,
      field: 'delete',
      onAction: (row: LoadAllocationConditionInfo) => this.onDelete(row),
    },
  ];
  ...
  readonly autoCompleteInputs: BaseAutoCompleteFilter<any>[] = [
    ...
    this.autoCompleteFactory.create(
      ...
      this.ctrl('TurnStatusId'),
      { control: this.ctrl('TurnStatusTitle') }
    ),
  ];

  /* -------------------------------- LIFECYCLE -------------------------------- */
  onViewActivated(): void {
    this.loadLoadAllocations();
  }

  /* ---------------------------------- LOAD DATA ---------------------------------- */
  private async loadLoadAllocations() {
    await this.withLoading(async () => {
      ...
      this.loadsAllocations.set(rows);
    });
  }

  /* -------------------------------- CRUD ACTIONS -------------------------------- */
  private async onDelete(row: LoadAllocationConditionInfo) {
    this.confirmService.confirmDelete(
      `شرایط انتخاب بار با شناسه ${row.LoadAllocationConditionId}`,
      async () => {
        if (this.loading()) return;

        await this.withLoading(async () => {
          const res = await this.configService.DeleteLoadAllocationCondition({
            LoadAllocationConditionId: Number(row.LoadAllocationConditionId),
          });
        ...
        await this.loadLoadAllocations();
      }
    );
  }
  ...
  private async submitEdit() {
    const selectedLoadAllocations = this.selectedLoadAllocations();
    if (!selectedLoadAllocations) return;

    const payload = this.serializeLoadAllocations({
      ...selectedLoadAllocations,
      ...(this.form.getRawValue() as LoadAllocationConditionInfo),
    });

    const res = await this.configService.EditLoadAllocationCondition(payload);
    if (!checkAndToastError(res, this.toast)) return;
    this.toast.success('موفق', res.data.Message);
  }
  ...
  private async submitRegister() {
    ...
    const { LoadAllocationConditionId: _, ...payload } = registerPayload;

    const res =
      await this.configService.RegisterLoadAllocationCondition(payload);
    if (!checkAndToastError(res, this.toast)) return;
    this.toast.success('موفق', res.data.Message);
  }

  /* ---------------------------------- UTILITIES ---------------------------------- */
  serializeLoadAllocations(
    obj: LoadAllocationConditionInfo
  ): LoadAllocationConditionInfo {
    return {
      LoadAllocationConditionId: Number(obj.LoadAllocationConditionId),
      ...
      TurnStatusId: Number(obj.TurnStatusId),
      TurnStatusTitle: obj.TurnStatusTitle.trim(),
    };
  }
  ...
  onNew() {
    ...
    this.selectedLoadAllocations.set(null);
    this.isDialogVisible = true;
    const title = 'ثبت شرایط انتخاب بار';
    this.dialogTitle.set(title);
  }

  onEdit(row: LoadAllocationConditionInfo) {
    this.form.reset();
    this.form.patchValue({ ...row });
    this.formMode.set(FormMode.EDITABLE);
    this.selectedLoadAllocations.set(row);
    this.isDialogVisible = true;
    const title = 'ویرایش شرایط انتخاب بار';
    this.dialogTitle.set(title);
  }
}
